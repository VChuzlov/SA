...
function runge_kutt(
  ...
  function sum(arr1, arr2: array of real; a: real): array of real;
  begin
    SetLength(result, arr1.Length);
...
begin
  ...
  begin
    ...
    k1 := func(time, c_, k);
    k2 := func(time + h / 2, sum(c_, k1, h / 2), k);
    k3 := func(time + h / 2, sum(c_, k2, h / 2), k);
    k4 := func(time + h, sum(c_, k3, h), k);
...
begin
 ...
 foreach var (i, el) in ReadLines('act_values.txt').Numerate do
 begin
   SetLength(act_values, i);
   act_values[i-1] := el.ToReal
 end;
 
 var k := genetic_algorithm(||1e-3, 10|, |1e-3, 10||, obj_func, act_values)[0];
 k.Println;
 
 var res := runge_kutt(kin_scheme, |1.0, 0.0, 0.0, 0.0|, k[:^1], 0.0, 1.0);
