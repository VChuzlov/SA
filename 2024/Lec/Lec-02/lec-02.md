...
- Другой способ определения словаря – передача последовательности пар (*ключ*, *значение*) в конструктор `dict()`. 
- Если ключами являются простые строки (которые могут использоваться как имена переменных), то пары можно также определять как именованные аргументы для конструктора `dict()`:

```python
ordinal = dict([(1, 'First'), (2, 'Second'), (3, 'Third')])
mass = dict(Mercury=3.301e23, Venus=4.867e24, Earth=5.972e24)
print(ordinal[2])  # Second
