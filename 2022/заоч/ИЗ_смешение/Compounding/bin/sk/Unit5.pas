...
var
  ...
  Dan:TDanFile;
  Rst:ResT;
  Ustanovka:string;      {Название установки}
  PolnoeIma:string;  {Полное имя выбранного файла с данными}
  Puti:string;       {Путь}
  PathS:string;      {Начальная директория}
  FrPath:string;
  PathU,PathIsl: string;
  DataOtbora,LastData:string; {Дата отбора}
  Process:string;    {Выбранный расчет процесса}
  N_Process:integer; {Номер выбранного процесса}

  DaOt:string[10];   {Дата отбора}
  DaRe:string[10];   {Дата регенерации катализатора}
  DiClor1, DiClor2, DiClor3:real;  {Подача хлорорганики}
  Akt:real;         {Активность катлизатора}
  P2, P3, P4, P5:real;          {Давление в Р-2, P-3, P-4, P-5}
  T2, T3, T4, T5:real;          {Температура в Р-2, P-3, P-4, P-5}
  dT2, dT3, dT4, dT5:real;      {Перепад температуры в Р-2, P-3, P-4, P-5}
  Vl:real;  {Влажность ВСГ}
  Rs:real;  {Расход сырья}
  Rg:real;  {Расход ВСГ}
  Sera:real;        {Количество серы}
  Vsir:real;        {Объем переработанного сырья, т}

  C1, C2, C3, C4, C5, C6, C7, C8, C9:real;      {Значения состава ВСГ}
  fr1:array [1..11] of real; {Значения фракционного состава}
  pl1:real; {Плотность сырья}
  Ce, Cr:real;    {Концентрация ароматики
                  экспериментальная
                  расчетная}

  CeStr, CrStr:string [8];    {Концентрация ароматики
                              экспериментальная
                              расчетная
                              строковые}

  CL1,CL2,CL3:real;           {Подача хлорорганики
                              по реакторам Р-202б Р-203Б Р-204}

  CL1Str,CL2Str,CL3Str:string [8]; {Подача хлорорганики
                                   по реакторам Р-202б Р-203Б Р-204
                                   строковые}

  Ak:real;                         {Уровень активности
                                   катализатора из "REZ"}

  AkStr:string [8];                {Уровень активности
                                   катализатора из "REZ"
                                   строковый}

  CLKat1,CLKat2,CLKat3:string[5];       {Хлора на катализаторе
                                   в Р-202. 203 ,204}

  CLKat1Str,CLKat2Str,CLKat3Str:string [8];   {Хлора на катализаторе
                                              в Р-202. 203 ,204
                                              строковые}
  ComL,DirL:array[0..79] of Char;
  w:double;
  procedure gopa(PolnoeIma:string;var Put,Gopen:string);
           {Процедура сортировки из полного имени
             выбранного файла даты и пути}
 procedure WriteDan(Dan:TDanFile;file2:String);
 procedure ReadDan(var Dan:TDanFile;file1:String);
function DelS(s: string): string;

           {Процедура расчета риформинга}
...
Begin
             ...
             Writeln(f,'       Џ®¤ з  ¤Ёе«®анв ­  Ї® аҐ Єв®а ¬ (ђ-202, ђ-203, ђ-204), аа¬.');
             WriteLn(f,Dan.Akt:6:2, '      ЂЄвЁў­®бвм Є в «Ё§ в®а  (гб«®ў­ п Ґ¤Ґ­Ёж )');
               For i:=1 to ColReakt do
                 Write(f,Dan.Pressure[i]:6:2,' ');
               If ColReakt=3 then Write(f,10);
             Writeln(f,'      „ ў«Ґ­ЁҐ Ї® аҐ Єв®а ¬ (ђ-202, ђ-203, ђ-204, ђ-205),  в¬.');
             Writeln(f,Dan.Vl:6:2,'      ‚« ¦­®бвм ‚‘ѓ, %');
               For i:=1 to Colreakt do
                 Write(f,Dan.Temperature[i]:6:2,' ');
               If ColReakt=3 then Write(f,400);
             Writeln(f,'      ’Ґ¬ЇҐа вга  ўе®¤­ п Ї® аҐ Єв®а ¬ (ђ-202, ђ-203, ђ-204, ђ-205), Ја ¤. ‘');
             For i:=1 to Colreakt do
                 Write(f,Dan.DifTemp[i]:6:2,' ');
               If ColReakt=3 then Write(f,10);  
             Writeln(f,'      ЏҐаҐЇ ¤л вҐ¬ЇҐа вга Ї® аҐ Єв®а ¬ (ђ-202, ђ-203, ђ-204, ђ-205), Ја ¤. ‘');
             For i:=1 to 9 do
                 Write(f,Dan.C[i]:6:5,' ');
             Writeln(f,'         ‘®бв ў ‚‘ѓ (H2, C1, C2, C3, n-C4, i-C4, n-C5, i-C5, C6), %');
             Writeln(f,Dan.Rg:6:2,'      ђ бе®¤ ‚‘ѓ, ¬3/з б');
             Writeln(f,Dan.Vsir:6:2,'      „­Ґ© Ї®б«Ґ аҐЈҐ­Ґа жЁЁ');
             Writeln(f,Dan.Sera:6:2,'      ‘Ґа  ў ЈЁ¤а®ЈҐ­Ё§ вҐ');
             Writeln(f,Dan.Rs:6:2,'      ђ бе®¤ бламп, ¬3/з б');
             Writeln(f,FormatDateTime('dd.mm.yy',Dan.DaOt),'             „ в  ®вЎ®а ');
             Writeln(f,FormatDateTime('dd.mm.yy',Dan.DaRe),'               „ в  аҐЈҐ­Ґа жЁЁ Є в «Ё§ в®а ');
             CloseFile(f);
...
begin
     OpenDialog1.Filter:='Технологические условия (*.dan)|*.dan';
     OpenDialog1.Title:='Открытие файла с данными';
...
//            gopa(PolnoeIma,Puti,DataOtbora);
     {Считываем технологические условия}
     ...
     If UstNum=11 then
     Readln(f,newP4,newT4,newdT4);     // считывание данных для 4 реактора
...
var ResForm6:integer; {Результат проверки выбора Формы 6
                      Согласен пользователь проводить дальнейший
                      расчет или нет}
    ResForm8:integer;
    Fs:File of ResT;
    CurDat:PResT;
...
{               AssignFile(Fs,PathS+'\svod.'+intToStr(Form4.RadioGroup1.ItemIndex+1));
     ...
     {Вызываем окно с технологическими условиями
     ...
     If ResForm8=mrOK then Begin
              ...
              Unit5.Form5.Label1.Caption:=' Пожалуйста подождите.             Идет расчет.';
              Unit5.Form5.Refresh;

             {Создаем резервный файл с
              измененными технологическими условиями}
             ...
             {производим расчет с измененными технологическими условиями}
    ...
    Case Form4.RadioGroup1.ItemIndex of
     ...
     9,16:   CoolActiv.ProcessName:=PutiProgram+'\ss99-10.exe'
             ...
             If CoolActiv.CalcComplited then Begin
                ...
                CrazyProcedure;
                NewReportForm.Label1.Caption:='Расчет проведен успешно.';
                NewReportForm.Label2.Caption:='На текущую дату '+DataOtbora+
                     ' переработанно '+FloatToStr(Dan.Vsir)+' тонн сырья.'+#13+
                     'Текущая активность катализатора составила '+
                     FloatToStr(CoolActiv.Results.akt)+' условных едениц.'+#13+
                     'Выход риформата: '+FloatToStr(CoolActiv.Results.Vyh)+'% вес.'+#13+
                     'Расчетная и экспериментальная концентрация ароматики в катализате: '+
                     FloatToStr(CoolActiv.Results.Arom[1,11])+' и '+FloatToStr(CoolActiv.Results.Arom[2,11])+
                     ' % вес. соответственно.';
                If Form4.RadioGroup1.ItemIndex<>8 then
                     NewReportForm.Label2.Caption:=NewReportForm.Label2.Caption+#13+
                     'Октановое число катализата: '+FloatToStr(CoolActiv.Results.Ochm)+' (M.M.), '+
                     FloatToStr(CoolActiv.Results.Ochi)+' (И.М.).';
...
Frak1.Label5.Caption:='Дата последнего отбора:  '+LastData;
frak1.Showmodal;
end
else ShowMessage('Нет данных по фракционному составу!');
end;
...
opendialog1.InitialDir:=PathU;
OpenDialog1.FileName:='';
opendialog1.Filter:='файлы результатов|*.rez';
...
begin
 Unit10.Form10.MemoRez.Lines.LoadFromFile(PathS+'\помощь.txt');
...
{******************************************************************************}
{******************************************************************************}
{Организация выбора каталога с результатами}
{******************************************************************************}
{******************************************************************************}
...
var    i:integer;
   stroka:string;
   Par:string;
   ListRez:TstringList;     {Считывавм файл "REZ"}
   ...
   s4:string [25];          {Строка при считывании
  ...
  CloseFile(f);
     ...
     Form8.koks3.Visible:=True;
             ...
             If (Way=1) and not(Form4.RadioGroup1.ItemIndex=8) then Begin
                InputQuery('Исследование активности','Введите ОЧИ за '+DataOtbora,par);
                CoolActiv.ExperimentalOctan:=StrToFloat(par);
                CoolActiv.TypeOfCalculation:=tcOctan;
                                                                    End;
             If Way=1 then Begin
                ss1:='Исследование оптимальной активности';
                ss2:='Оптимальная активность';
                           end;
             ...
             If Way=2 then begin
                ...
                ss1:='Исследование влияния состава сырья';
                ss2:='Активность';
    ...
    Case Form4.RadioGroup1.ItemIndex of
     ...
     9,16:   CoolActiv.ProcessName:=PutiProgram+'\ss99-10.exe'
             ...
             If CoolActiv.CalcComplited then Begin
                ...
                NewReportForm.Label2.Caption:='На текущую дату '+DataOtbora+
                     ' переработанно '+FloatToStr(Dan.Vsir)+' тонн сырья.'+#13+
                     ss2+' катализатора составила '+
                     FloatToStr(CoolActiv.Results.akt)+' условных едениц.';
                If Form4.RadioGroup1.ItemIndex<>8 then
                     NewReportForm.Label2.Caption:=NewReportForm.Label2.Caption+#13+
                     'Октановое число катализата: '+FloatToStr(CoolActiv.Results.Ochm)+' (M.M.), '+
                     FloatToStr(CoolActiv.Results.Ochi)+' (И.М.).';
...
Until (not Go) or (MessageDlg('Результаты сохранены в файле:'+#13+
        'результаты расчета                  '+ResF.RezFile+#13+
        'Продолжить исследования?',mtConfirmation,[mbYes,mbNo],0)=mrNo)
end;
...
Report.BeginUpdate;
Report.Add('Отчет по установке '+Ustanovka+' сравнение по датам '+Form12.Label25.Caption+'/'+Form12.Label24.Caption);
...
Report.Add(' ');
Report.Add('ИСХОДНЫЕ ДАННЫЕ');
Report.Add(' ');
Report.Add('Содержание водорода в ВСГ       '+Form12.Label17.Caption+'/'+Form12.C3.Caption);
Report.Add('Расход сырья                    '+Form12.Label18.Caption+'/'+Form12.C4.Caption);
Report.Add('Отношение н-Р/и-Р:');
Report.Add('      -в сырье                  '+Form12.Label22.Caption+'/'+Form12.C7.Caption);
Report.Add('      -в катализате             '+Form12.Label21.Caption+'/'+Form12.C8.Caption);
Report.Add('Объем переработанного сырья, т  '+Form12.Label27.Caption+'/'+Form12.C11.Caption);
Report.Add(' ');
...
Report.Add(' ');
Report.Add('РЕЗУЛЬТАТЫ РАССЧЕТА');
Report.Add(' ');
//Report.Add('Дата отбора                     '+Form12.Label25.Caption+'/'+Form12.Label24.Caption);
Report.Add('Активность катализатора         '+Form12.Label15.Caption+'/'+Form12.C1.Caption);
Report.Add('Число крекинга                  '+Form12.Label16.Caption+'/'+Form12.C2.Caption);
Report.Add('Степень превращения парафинов в реакции:');
Report.Add('      -Изомеризации             '+Form12.Label19.Caption+'/'+Form12.C5.Caption);
Report.Add('      -Дегидроциклизации        '+Form12.Label20.Caption+'/'+Form12.C6.Caption);
Report.Add('Выход риформата, %масс.         '+Form12.Label23.Caption+'/'+Form12.C9.Caption);
Report.Add('Содержание МЦП в катализате     '+Form12.Label12.Caption+'/'+Form12.C10.Caption);
Report.Add('Октановое число, о.ч.и.         '+Form12.Label29.Caption+'/'+Form12.C12.Caption);
...
begin
PathIsl:=PathU+'\Issl';
ResF.Caption:='Исследование влияния состава сырья';
...
ResF.Caption:='Расчет оптимальной активности';
If not DirectoryExists(PathIsl) then ForceDirectories(PathIsl);
RasIssl(1);
end;
