...
begin
  var res := rk(kin_scheme, |1.0, 1.0, 0.0, 0.0|, k, 0.0, 1.0);
  var res1d := res[10][1:];
...
begin
  ...
  foreach var (i, val) in ReadLines('var10.txt').Numerate do
  begin
    SetLength(act_val, i);
  ...
  println;
  var res := rk(kin_scheme, |1.0, 1.0, 0.0, 0.0|, k[:^1], 0.0, 1.0);
  println('Время, с', '  C[0]', '    C[1]', '    C[2]', '    C[3]');
  foreach var line in res[::10] do
  begin
