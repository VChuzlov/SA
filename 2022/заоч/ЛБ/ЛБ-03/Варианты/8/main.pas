...
begin
  var res := rk(kin_scheme, |1.0, 1.0, 0.0, 0.0|, k, 0.0, 5.0);
  var res1d := res[50][1:];
  foreach var i in range(100, res.High, 50) do
    res1d := res1d + res[i][1:];
...
begin
  ...
  foreach var (i, val) in ReadLines('var8.txt').Numerate do
  begin
    SetLength(act_val, i);
  ...
  println;
  var res := rk(kin_scheme, |1.0, 1.0, 0.0, 0.0|, k[:^1], 0.0, 5.0);
  println('Время, с', '  C[0]', '    C[1]', '    C[2]', '    C[3]');
  foreach var line in res[::50] do
  begin
