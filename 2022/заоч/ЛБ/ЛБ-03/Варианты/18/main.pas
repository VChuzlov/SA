...
begin
  var res := rk(kin_scheme, |1.0, 1.0, 0.0, 0.0|, k, 0.0, 3.5);
  var res1d := res[35][1:];
  foreach var i in range(70, res.High, 35) do
    res1d := res1d + res[i][1:];
...
begin
  ...
  foreach var (i, val) in ReadLines('var18.txt').Numerate do
  begin
    SetLength(act_val, i);
  ...
  println;
  var res := rk(kin_scheme, |1.0, 1.0, 0.0, 0.0|, k[:^1], 0.0, 3.5);
  println('Время, с', '  C[0]', '    C[1]', '    C[2]', '    C[3]');
  foreach var line in res[::35] do
  begin
