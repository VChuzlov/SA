...
// Потоки: ГОБКК,КТ-1/1,Риф.1000,Риф.600,Толуол,Изомеризат,Изопентан,Алкилат,МТБЭ,фр.62-85 АВТ-10,фр.62-85 С100 КПА,Рафинат С400 КПА
...
Rash: TArrOfDouble;  // килограммы овлекаемых потоков в смесь
iter:integer; // количество итерации
...
Function RashSera(TrebSera,FaktSera:real;i:integer):real; 
 ...
 If Sera[i]=0 then              //если в потоке нет компонента значит на этот показатель данный поток не влияет
   RashSera:=1;
 end;
...
function RashDNPmin(TrebDNPmin,FaktDNP:real;i:integer):real;
 begin
  ...
  //если возникнет ситуация когда ДНП потока равна нулю нужно что то с этим делать... сейчас не стал думать
 end;
...
var
 ...
 begin
  ...
  // тут надо добавлять мое обращение
n:=length(TotalFlComp.Flows);

For i:=1 to n do
    FaktKolVo:=FaktKolVo+Rash[i];     //количество килограм суммарного потока
...
begin
  if TotalFlComp.GetFlowOfIndexOfCondition(ii)<>nil then
    ...
    Rashodi[ii]:=0; // Должно считываться, для отладки задали количество постоянным
end;
...
// проверка по излишкам
AllTreb(TotalFlComp, FaktOch,FaktDNP,FaktBenzol,FaktArom,FaktOlef,FaktSera,FaktKolVo);
// если есть возможность добавлять КТ1-1 то добавлять ести нет то следующий шаг
          ...
          IF FaktKolVo+Rash[8]>1 then
             If Rash[1]>0 then
                begin
                  ...
                  //условия по уменьшению количества ГОБКК
                    If Rash[2]=0 then
                      begin
                       If (Rash[1]>0) then
                          Rash[1]:=Rash[1]-0.01;
...
// добавление прямогонных фракций
   // 62-85 АВТ-10
 If (FaktOch>=TrebOch[indCond]) and (FaktKolVo-1<0) and (Rash[10]<Rashodi[10]/TrebKolVo[indCond]) then
     begin
       ...
       //writeln('62-85 АВТ-10=',Rash[10]:5:2);
       AllTreb(TotalFlComp, FaktOch,FaktDNP,FaktBenzol,FaktArom,FaktOlef,FaktSera,FaktKolVo);
       //writeln(FaktOch:5:2,' ',FaktDNP:5:2,' ',FaktBenzol:5:2,' ',FaktArom:5:2,' ',FaktOlef:5:2,' ',FaktSera:5:2,' ',FaktKolVo:5:2);
 ...
 If (FaktOch>=TrebOch[indCond]) and (FaktKolVo-1<0) and (Rash[11]<Rashodi[11]/TrebKolVo[indCond]) then
     begin
       ...
       //writeln('62-85 C100 КПА=',Rash[11]:5:2);
       AllTreb(TotalFlComp, FaktOch,FaktDNP,FaktBenzol,FaktArom,FaktOlef,FaktSera,FaktKolVo);
       //writeln(FaktOch:5:2,' ',FaktDNP:5:2,' ',FaktBenzol:5:2,' ',FaktArom:5:2,' ',FaktOlef:5:2,' ',FaktSera:5:2,' ',FaktKolVo:5:2);
 ...
 //рафинат С400
  If (FaktOch>=TrebOch[indCond]) and (FaktKolVo-1<0) and (Rash[12]<Rashodi[12]/TrebKolVo[indCond]) then
     begin
       ...
       //writeln('рафинат С400=',Rash[12]:5:2);
       AllTreb(TotalFlComp, FaktOch,FaktDNP,FaktBenzol,FaktArom,FaktOlef,FaktSera,FaktKolVo);
       //writeln(FaktOch:5:2,' ',FaktDNP:5:2,' ',FaktBenzol:5:2,' ',FaktArom:5:2,' ',FaktOlef:5:2,' ',FaktSera:5:2,' ',FaktKolVo:5:2);
...
If (FaktKolVo-1<0) or (FaktOch-TrebOch[indCond]>0.2) or (FaktDNP>TrebDNPmax[indCond]) or
   ...
   (FaktOlef>TrebOlef[indCond]) or (FaktSera>TrebSera[indCond]+0.001) then
      ...
      // If TrebOch[indCond]<90 then
          begin
            //условия по уменьшению количества риформинг 600
               If Rash[3]=0 then 
                begin
                   If (Rash[4]>0) then
                     Rash[4]:=Rash[4]-0.01;  
            ...
            //условия по уменьшению количества риформинг 1000
               If Rash[5]=0 then 
                begin
                   If (Rash[3]>0) then
                     Rash[3]:=Rash[3]-0.01;  
          ...
          //условия по уменьшению количества Толуола
               If Rash[1]=0 then
                begin
                   If (Rash[5]>0) then
                     Rash[5]:=Rash[5]-0.01;  
          ...
          //условия по уменьшению количества КТ 1/1
                 If (Rash[2]>0) then
                   Rash[2]:=Rash[2]-0.01;  
                 IF Rash[2]<0 then
                    Rash[2]:=0;
...
begin
  CreateMessageDialog('Ты дурак? Просишь больше, чем даешь..', mtConfirmation, [mbYes, mbNo]).ShowModal;
  exit;
end; }
